...
describe("GET /api/trips/:id/transport", () => {
  beforeEach(() => {
    fakeDb = createFakeFirestore();
  });

  it("returns the transport summary for a trip the user owns", async () => {
    const app = createApp();
    const created = await request(app).post("/api/trips").set(authed("token-user-1")).send(validBody);

    const res = await request(app)
      .get(`/api/trips/${created.body.trip.id}/transport`)
      .set(authed("token-user-1"));

    expect(res.status).toBe(200);
    expect(res.body.transport.distanceKm).toBe(582.3);
    expect(res.body.transport.options).toHaveLength(3);
    expect(res.body.transport.options[0]).toMatchObject({ mode: "flight", recommended: true });
  });
  ...
  it("rejects requests with no Authorization header", async () => {
    const app = createApp();
    const res = await request(app).get("/api/trips/some-id/transport");
